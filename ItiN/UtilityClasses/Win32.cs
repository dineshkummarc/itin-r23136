...
namespace ItiN
{
  ...
  public sealed class NativeMethods
  {
    ...
    internal const int EM_GETTEXTLENGTHEX = 0x045f;
    internal const int EM_GETTEXTEX = 0x045e;
    ...
    [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Auto)]
    internal static extern IntPtr GetWindow(IntPtr handleToWindow, int wParam);
    ...
    /// <param name="hwnd">The handle to the window</param>
    /// <returns>Text of the window</returns>
    internal static string GetDialogText(IntPtr hwnd)
    {
        IntPtr detailsButton = GetWindow(hwnd, GW_CHILD);

        while (detailsButton != IntPtr.Zero)
        {
            detailsButton = GetWindow(detailsButton, GW_HWNDNEXT);
            string windowClass = (new Window(detailsButton)).ClassName;
            if (windowClass == "Button" && GetWindowText(detailsButton) == "Show &Details >>")
            {
                WinButton showDetailsButton = new WinButton(detailsButton);
                showDetailsButton.Click();
            }
        }

        IntPtr textWindow = GetWindow(hwnd, GW_CHILD);

        string textContent = String.Empty;
        while (textWindow != IntPtr.Zero)
        {
            textWindow = GetWindow(textWindow, GW_HWNDNEXT);
            string windowClass = (new Window(textWindow)).ClassName;
            if (windowClass == "RichEdit20W")
            {
                textContent += GetRichText(textWindow);
            }
            else if(windowClass == "MSOUNISTAT")
            {
                textContent += GetWindowText(textWindow);
            }
        }
        return textContent;
    }
      ...
      private static string GetRichText(IntPtr hwnd)
      {
          int length = SendMessage(hwnd, WM_GETTEXTLENGTH, 0, 0) + 1;
          StringBuilder buffer = new StringBuilder(length);
          SendMessage(hwnd, WM_GETTEXT, length, buffer);
          return buffer.ToString();
      }
    /// <summary>
    /// This method incapsulates all the details of getting
