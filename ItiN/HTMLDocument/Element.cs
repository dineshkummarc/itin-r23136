...
namespace ItiN
{
    ...
    public class Element
    {
        ...
        public object HTMLElement
        {
            get
            {
                if (!ElementAvailable())
                {
                    ...
                    catch (ItiN.Exceptions.TimeoutException)
                    {
        ...
        /// To change the default time out, set <see cref="P:ItiN.Settings.WaitUntilExistsTimeOut"/>
        /// </summary>
        public void WaitUntilExists()
        {
            // Wait 30 seconds max
            WaitUntilExists(InfopathTester.Settings.WaitUntilExistsTimeOut);
        }
        ...
        {
            // Wait 30 seconds max
            WaitUntilRemoved(InfopathTester.Settings.WaitUntilExistsTimeOut);
        }
        ...
        {
            WaitUntil(attributename, expectedValue, InfopathTester.Settings.WaitUntilExistsTimeOut);
        }
        ...
        private static void ThrowTimeOutException(Exception lastException, string message)
        {
            ...
            {
                throw new ItiN.Exceptions.TimeoutException(message, lastException);
            }
            else
            {
                throw new ItiN.Exceptions.TimeoutException(message);
            }
        }
